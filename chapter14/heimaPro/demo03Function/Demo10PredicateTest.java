...
/*
 * 数组当中有多条"姓名+种族"的信息，通过Predicate接口的拼装，将符合要求的字符串筛选到集合ArrayList中，需要满足以下条件：
 * 1、必须是龙
 * 2、姓名为3个字
 *
 * 分析：
 * 1、有两个判断条件，所以使用两个Predicate接口，对条件进行判断
 * 2、必须同时满两个条件，所以可以使用and方法连接两个判断条件
 * */
public class Demo10PredicateTest {
    /*
     * 定义一个方法
     * 方法的参数传递一个包含人员信息的数组
    ...
    public static void main(String[] args) {
        //定义一个存储字符串的数组
        String[] array = {"苍蓝星,人", "咩咩子,龙", "雷娘,龙", "弟弟龙,龙"};
        //调用filter方法，传递字符串数组和两个Lambda表达式
        ArrayList<String> list = filter(array, (String str) -> {
            //获取字符串中的种族，判断是否为龙
            return str.split(",")[1].equals("龙");
        }, (String str) -> {
            //获取字符串中的姓名，判断长度是否为3个字符
